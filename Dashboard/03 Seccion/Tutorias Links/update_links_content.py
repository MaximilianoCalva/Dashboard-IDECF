#!/usr/bin/env python3
"""
Script para actualizar los archivos de Tutorias Links con el HTML completo del reproductor
"""

import os

# Template HTML completo (igual que en Tutorias Grabadas pero sin Elementor)
HTML_TEMPLATE = '''<!-- Tailwind CSS -->
        ...
        <div class="w-full lg:w-2/3 flex flex-col">
            ...
            <div class="mt-4 px-2">
                ...
                <div class="flex items-center gap-3 mt-1">
                    <p id="mainDate" class="text-sm text-gray-500 font-medium">📅 --/--/----</p>
                    <span class="text-gray-300">|</span>
                    <p id="mainDocente" class="text-sm text-[#6D0757] font-medium">👨‍🏫 --</p>
                </div>
            </div>
        </div>
        ...
        <div
            ...
            <div class="sticky top-0 bg-white p-4 z-10 border-b border-gray-200 shadow-sm">
                <div class="relative">
                    <input type="text" id="searchInput" placeholder="🔍 Buscar clase..."
                        class="w-full pl-10 pr-4 py-2 bg-gray-50 rounded-lg border border-gray-200 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#6D0757] focus:border-transparent text-sm transition-all"
            ...
            <div class="playlist-items overflow-y-auto flex-1 custom-scrollbar" id="playlistItems">
                <div class="p-8 text-center flex flex-col items-center justify-center h-full text-gray-400">
                    <svg class="animate-spin h-8 w-8 mb-3 text-[#6D0757]" xmlns="http://www.w3.org/2000/svg" fill="none"
                        viewBox="0 0 24 24">
...
<style>
    ...
    .playlist-item.active {{
        ...
        /* blue-50 */
        border-left: 4px solid #6D0757;
    }}
</style>

<script>
    // URL del Apps Script de IDECF
    const WEB_APP_URL = "https://script.google.com/macros/s/AKfycbzbr5gMZQGnY6ZHVncD0JUsgB0jQ97KOaVwcF8dez0vax37o4VXqvW0KhJnPNhuWAI_sA/exec";

    // CAMBIAR AQUÍ EL NOMBRE DE LA PESTAÑA QUE QUIERES CARGAR EN ESTE HTML
    ...
    async function loadPlaylist() {{
        try {{
            ...
            // Filtrar solo la pestaña deseada
            if (data[SHEET_NAME]) {{
                // Usar orden original del Sheet pero INVERTIDO y FILTRANDO encabezados repetidos
                allVideos = data[SHEET_NAME].filter(v => v.titulo && v.titulo !== "Título del Video").reverse();

                renderItems(allVideos);

                if (allVideos.length > 0) {{
                    // Para el player usamos el link directo
                    updatePlayer(allVideos[0].vimeoUrl, allVideos[0].titulo, allVideos[0].fecha, allVideos[0].profesor);
                ...
                document.getElementById('playlistItems').innerHTML = `<p class="p-4 text-red-500 text-center">No se encontró la pestaña "${{SHEET_NAME}}"</p>`;
            }}

        }} catch (error) {{
            console.error("Error cargando la lista:", error);
            document.getElementById('playlistItems').innerHTML = `<p class="p-4 text-red-500 text-center">Error de conexión con la base de datos.</p>`;
        }}
    }}

    // Función auxiliar para extraer ID de URL completa (Solo para miniaturas)
    function getVimeoId(url) {{
        if (!url) return null;
        // Si es un número (ID directo del sheet)
        if (typeof url === 'number' || !isNaN(url)) return url.toString();
        // Si es string
        const match = url.toString().match(/vimeo.*\\/(\\d+)/i);
    ...
    function renderItems(videos) {{
        ...
        videos.forEach((video, index) => {{
            ...
            item.onclick = function () {{
                ...
                updatePlayer(video.vimeoUrl, video.titulo, video.fecha, video.profesor);
            ...
            // Badge para el primer video (El más reciente si la hoja está en orden cronológico)
            ...
            item.innerHTML = `
                ...
                <div class="flex-1 min-w-0 flex flex-col justify-center py-1">
                    <div class="text-xs font-semibold text-[#6D0757] mb-1 opacity-70 flex items-center">
                        <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                        ${{video.fecha}}
                    </div>
                    <div class="text-xs font-medium text-gray-500 mb-1">👨‍🏫 ${{video.profesor || 'Docente no especificado'}}</div>
                    <div class="text-sm font-bold text-gray-700 leading-tight line-clamp-2 group-hover:text-[#6D0757] transition-colors">${{video.titulo}}</div>
                </div>
            `;
    ...
    function filterPlaylist() {{
        ...
        const filtered = allVideos.filter(video =>
            ...
            (video.profesor && video.profesor.toLowerCase().includes(query))
        );
        renderItems(filtered);
    }}

    function updatePlayer(fullUrl, title, date, profesor) {{
        // Aseguramos que el link sea embeddable (player.vimeo.com)
        let embedUrl = fullUrl;

        // Si es un link normal de vimeo.com (no player.vimeo.com), o es un ID numérico
        const vId = getVimeoId(fullUrl);
        if (vId) {{
            embedUrl = `https://player.vimeo.com/video/${{vId}}`;
        }} else if (fullUrl && fullUrl.toString().includes('player.vimeo.com')) {{
            embedUrl = fullUrl;
        }}
        ...
        const docenteEl = document.getElementById('mainDocente');
        if (docenteEl) docenteEl.innerText = `👨‍🏫 ${{profesor || 'Docente no disponible'}}`;
...
def update_tutorial_links():
    """Actualiza los archivos de Tutorias Links con el HTML completo"""
    ...
    for i in range(1, 21):
        sheet_name = f"DCFG{i:02d}"
        filename = f"dcfg{i:02d}.html"
        filepath = os.path.join(dcf_path, filename)
        
        html_content = HTML_TEMPLATE.format(sheet_name=sheet_name)
    ...
    print("\n📁 Updating DTG files...")
    for i in range(1, 21):
        sheet_name = f"DTGG{i:02d}"
        filename = f"dtgg{i:02d}.html"
    ...
    print(f"\n🎉 Successfully updated 40 tutorial link files!")
    print(f"\n📝 These files now contain the full HTML player and will work at:")
    print(f"   https://panel.idecf.com/dcfg01/ ... dcfg20/")
    print(f"   https://panel.idecf.com/dtgg01/ ... dtgg20/")

if __name__ == "__main__":
    update_tutorial_links()
